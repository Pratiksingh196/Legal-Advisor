...
export default function VideoMeetComponent() {
    ...
    // If already spotlighted, unselect
    if (spotlight === socketId) setSpotlight(null);
    else setSpotlight(socketId);
  };

    const { meetingId } = useParams();
  const location = useLocation();
  const { role, lawyerId } = location.state || {};
  ...
  console.log("Duration:", duration);
    ...
    let connectToSocketServer = () => {
        ...
        socketRef.current.emit("set-meeting-duration", {
...
        window.location.href = "/";
        ...
        socketRef.current.on('connect', () => {
            ...
            socketRef.current.on('user-joined', (newSocketId, newUserName, allClients) => {
  console.log(`New user joined: ${newUserName} (ID: ${newSocketId})`);
  console.log('All clients in room:', allClients);

  // allClients is now an array of objects: [{ id, name }, ...]
  allClients.forEach((client) => {
    const socketListId = client.id;

    // Skip connection to yourself
    ...
    connections[socketListId].onaddstream = (event) => {
      ...
      if (existingVideo) {
        ...
        const newVideo = {
          ...
          autoplay: true,
          playsinline: true,
          name: client.name || newUserName || "Guest", // Optional: show name on video
        };
    ...
    if (window.localStream) {
      ...
      // Fallback silent/black stream (if user denied mic/cam)
  ...
  // If the new user is *you*, create offers to all others
    ...
    {askForUsername === true ? (
      <div className={styles.lobbyPage}>
        <div className={styles.lobbyBox}>
          ...
          <h3 style={{ marginTop: "20px", color: "#ccc" }}>OR</h3>
